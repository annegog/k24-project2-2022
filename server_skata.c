...
int find_the_files(char* file, pthread_t thread, Deque queue){
    DIR *folder;
    struct dirent *entry;
    char new_file[BUFF];
    folder = opendir(file);
    ...
    while( (entry=readdir(folder)) ){
        if (entry->d_type == DT_DIR){
            if( !(strcmp(entry->d_name,".") == 0 || strcmp(entry->d_name,"..") == 0) ){
                ...
                find_the_files(new_file,thread,queue);
            }
        }
        else{
            pos++;
...
void *communication_thread(void *argp){ 
    ...
    if(read(args->first, buff, BUFF) < 0){
        perror_exit("read");
    }
    ...
    //find_the_files(buff,pthread_self(), args->second);
...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        // for(DequeNode node = deque_first(queue_of_files);
        //     node != DEQUE_EOF; node = deque_next(queue_of_files, node)){
        //         printf("%s\n", (char*)deque_node_value(queue_of_files,node));
        // }
